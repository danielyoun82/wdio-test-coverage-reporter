...
class CustomReporter extends events.EventEmitter {
    constructor(baseReporter, config, options = {}) {
        ...
        this.mainEvents = new Array;
        this.testEvents = new Array;
        this.suiteEvents = new Array;
        this.resultCount = new Array;
        ...
        this.on('end', function (setting) {
            const overview = {
                ...
                'reportOutput':this.config.reporterOptions.CustomReporter.outputDir,
            ...
            for (const cid of Object.keys(this.baseReporter.stats.runners)) {
                ...
                for (const specId of Object.keys(runnerDetail.specs)) {
                    ...
                    for (const suiteName of Object.keys(specDetail.suites)) {
                        ...
                        if (!(suiteDetail.title.includes('"before all" hook') ||
                         suiteDetail.title.includes('"after all" hook'))) {
                            for (const testName of Object.keys(suiteDetail.tests)) {
                                ...
                                const testResult = this.buildTest(cid, suiteName, testDetail);
    ...
    write(json) {
        ...
        try {
            ...
            let jsonFilename = this.options.jsonFilename ? this.options.jsonFilename : 'result.json';

            jsonFilename = jsonFilename.indexOf('.json') >= 0 ? jsonFilename : jsonFilename + '.json'; // if filename is missing .json
    ...
    consolidateAdditionalSuiteInfo(allSuites, suiteEvents) {
        for (const suite of Object.keys(allSuites)) {
            for (const suiteEvent of Object.keys(suiteEvents)) {
                if (allSuites[suite].title === suiteEvents[suiteEvent].title &&
                allSuites[suite].file === suiteEvents[suiteEvent].file) {
    ...
    buildTest(cid, suiteName, testdata) {
        ...
        if (testdata.error) {
            ...
            if (typeof(testdata.error.actual) !== 'undefined' &&
             typeof(testdata.error.expected) !== 'undefined') {
    ...
    processTestEvents(testResult, events) {
        ...
        for (const index of Object.keys(events)) {
            if (testResult.cid === events[index].cid &&
            testResult.title === events[index].title &&
            testResult.parent === events[index].parent) {
    ...
    processMainEvents(suiteInfo, events) {
        ...
        for (const filename of Object.keys(suiteInfo)) {
            ...
            for (const index of Object.keys(events)) {
                if (currSuite.cid === events[index].cid &&
                currSuite.file === events[index].file) {
...
CustomReporter.reporterName = 'WdioTestCoverageReporter';
